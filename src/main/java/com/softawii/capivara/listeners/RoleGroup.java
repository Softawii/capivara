...
public class RoleGroup {
    ...
    @ICommand(name = "create", description = "Comando para criar um novo cargo",
              permissions = {Permission.ADMINISTRATOR})
    @IArgument(name = "name", description = "O nome do cargo a ser criado", type = OptionType.STRING, required = false)
    @IArgument(name = "channel", description = "O canal que o cargo será vinculado", type = OptionType.CHANNEL,
               required = false)
    @IArgument(name = "color", description = "A cor do cargo em hexadecimal (#FFFFFF)", type = OptionType.STRING,
               required = false)
    @IArgument(name = "visible",
               description = "Se o cargo deve ser exibido na lista de cargos separadamente, ali do lado sabe??",
               type = OptionType.BOOLEAN, required = false)
    @IArgument(name = "mentionable", description = "Se o cargo pode ser mencionado, ou seja, @everyone!",
               type = OptionType.BOOLEAN, required = false)
    ...
    @ICommand(name = "delete", description = "Quer deletar um cargo do servidor, amigao?",
              permissions = {Permission.ADMINISTRATOR})
    ...
    @ICommand(name = "clean", description = "Vamo fazer a limpa, hein?? Não vai sobrar ninguem!!",
              permissions = {Permission.ADMINISTRATOR})
    @IArgument(name = "role", description = "Fala onde vamos limpar pro seu pai aqui!", type = OptionType.ROLE,
               required = true)
    ...
    public static class Channel {
        @ICommand(name = "permissions", description = "Quer atualizar rapidamente algumas permissões do seu cargo??",
                  permissions = {Permission.ADMINISTRATOR})
        @IArgument(name = "role", description = "Fala o cargo ai chefe", type = OptionType.ROLE, required = true)
        @IArgument(name = "related", description = "Pra qual canal que tu quer atualizar???", type = OptionType.CHANNEL,
                   required = true)
        ...
        public static void permissions(SlashCommandInteractionEvent event) {
            ...
            if (channelType.isMessage()) {
                ...
                if (allow)
                    channel.getPermissionContainer().getManager().putRolePermissionOverride(role.getIdLong(), List.of(getPermission(permission)), null).queue();
                else
                    channel.getPermissionContainer().getManager().putRolePermissionOverride(role.getIdLong(), null, List.of(getPermission(permission))).queue();
            }
